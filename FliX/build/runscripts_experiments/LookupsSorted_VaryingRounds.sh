...
for Div in 8; do #may test varying DIV sizes for experiments; Div == 8  is optimal 
 ...
 # -DNODESIZE=3 -DMAX_NODE=8 -DDEFINE_TILE_SIZE=8 -DDENSEKEYGEN -DTILE_DELETES -DTILE_INSERTS -DTILE_INSERTS_C -DINSERTS_TILE_BULK_ONLY -DTOTALRUNS=1 -DMAIN_32 -DDIV=8 -DXVAL= -DYVAL=90 -DLOOKUPS_CUDA"
  BASE_IFDEFS_TILED="-DDENSEKEYGEN -DXVAL=${Xval} -DYVAL=${Yval} -DTOTALRUNS=5 -DMAIN_32 -DDIV=${Div} -DTILE_INSERTS -DTILE_INSERTS_C -DINSERTS_TILE_BULK_ONLY -DTILE_DELETES -DDELETES_TILE_BULK -DMAX_NODE=${MaxNode} -DDEFINE_TILE_SIZE=${TileSize}${NodeCacheString} -DINITIAL_BUILD_SIZE=${BUILDSIZE} -DINITIAL_PROBE_SIZE=${PROBESIZE} -DUPDATE_INSERT_PERCENT=${GrowthVal} -DUPDATE_DELETE_PERCENT=${GrowthVal}"
