...
# fi
  ...
  OUT_TXT="${OUT_BASENAME}+{GPULSMOpt}.txt"
  OUT_CSV="${OUT_BASENAME}+{GPULSMOpt}.csv"
  build_and_make "$BASE_IFDEFS_BASELINES" "-DGPULSMOPT -DROUNDS_NUMBER=${Rounds}"
  ./index_prototype &> "$OUT_TXT"
  cp updates.csv "$OUT_CSV"
  mv "${OUT_BASENAME}+"* "$TARGET_DIR"/
  CSV_FILES+=("${TARGET_DIR}/${OUT_CSV}")
  echo "GPULSMOpt complete."
