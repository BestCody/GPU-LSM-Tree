...
// Description: FliX benchmark adapter for the root-level GPULSMOpt implementation.
// =============================================================================
...
__global__ void fill_sequence_kernel(std::uint32_t* values, size_t size) {
    ...
    if (tid < size) values[tid] = static_cast<std::uint32_t>(tid);
}
...
__global__ void finalize_lookup_kernel(const std::uint8_t* found,
                                       ...
                                       size_t size) {
    const size_t tid = blockIdx.x * size_t(blockDim.x) + threadIdx.x;
    if (tid < size && found[tid] == 0) values[tid] = not_found;
}

inline void fill_sequence(std::uint32_t* values, size_t size, cudaStream_t stream) {
    if (size == 0) return;
...
inline void finalize_lookup(const std::uint8_t* found,
                            smallsize* values,
                            size_t size,
                            cudaStream_t stream) {
    if (size == 0) return;
    constexpr int threads_per_block = 256;
    const int blocks = static_cast<int>((size + threads_per_block - 1) / threads_per_block);
    finalize_lookup_kernel<<<blocks, threads_per_block, 0, stream>>>(found, values, size);
    check_cuda(cudaGetLastError());
}
...
public:
    ...
    static parameters_type parameters() {
        return {
            {"batch_size", std::to_string(static_cast<size_t>(GPULSMOPT_BATCH_SIZE))},
            {"segment_buckets", std::to_string(static_cast<size_t>(GPULSMOPT_SEGMENT_BUCKETS))},
            {"target_fill", std::to_string(static_cast<size_t>(GPULSMOPT_TARGET_FILL))},
            {"inplace_max_incoming", std::to_string(static_cast<size_t>(GPULSMOPT_INPLACE_MAX_INCOMING))},
        };
    }
    ...
    void build(const key_type* keys,
               size_t size,
               size_t max_size,
        ...
        if (configured_max_size > static_cast<size_t>(std::numeric_limits<std::uint32_t>::max() / 2)) {
            throw std::runtime_error("GPULSMOpt supports at most 2^31-1 records");
        }

        build_values_buffer_.resize(size);

        const size_t configured_batch_size = static_cast<size_t>(GPULSMOPT_BATCH_SIZE);
        const size_t config_batch_size = std::max<size_t>(1, std::min(configured_max_size == 0 ? size : configured_max_size,
        ...
        batch.values = reinterpret_cast<const std::uint32_t*>(build_values_buffer_.ptr());
        batch.count = size;
    ...
    void lookup(const key_type* keys, value_type* result, size_t size, cudaStream_t stream) {
        ...
        gpulsmopt_adapter_detail::finalize_lookup(lookup_found_buffer_.ptr(), result, size, stream);
    }
