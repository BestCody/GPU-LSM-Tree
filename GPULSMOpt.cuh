...
__global__ void ds_route_only_kernel(
  ...
  if (lo >= total_buckets)
    lo = total_buckets - 1;
  dest[i] = static_cast<std::uint32_t>(lo);
}

__global__ void ds_count_epoch_misses_kernel(
    const std::uint32_t *dest, const std::uint8_t *epoch_found,
    std::size_t n, std::uint32_t *cnt, std::uint32_t *dirty_list,
    std::uint32_t *counters) {
  const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n || epoch_found[i])
    return;
...
__global__ void ds_scatter_epoch_misses_kernel(
    const std::uint32_t *keys, std::size_t n,
    const std::uint32_t *dest, const std::uint8_t *epoch_found,
    const std::uint32_t *base, std::uint32_t *cursor,
    std::uint32_t *staged_keys) {
  const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n || epoch_found[i])
    return;
...
__global__ void ds_bucket_delete_kernel(
  ...
  const std::size_t first_warp =
      (static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x) /
      kBucketSlots;
  const std::size_t warp_stride =
      static_cast<std::size_t>(gridDim.x) * blockDim.x / kBucketSlots;
  const int lane = threadIdx.x & (kBucketSlots - 1);
  for (std::size_t w = first_warp; w < counters[0]; w += warp_stride) {
...
#pragma unroll
      for (int j = 0; j < kBucketSlots; ++j) {
        const std::uint32_t broadcast =
            __shfl_sync(0xffffffffu, del_key, j);
...
// Epochs are newest-first; buckets are sorted.
__device__ inline bool seg_point_lookup(
    std::uint32_t ord, std::uint32_t seg_id, std::uint32_t key,
...
__device__ inline std::uint32_t seg_range_count_one(
    std::uint32_t ord, std::uint32_t seg_id, std::uint32_t lo,
    std::uint32_t hi, const std::uint32_t *pool_keys,
    const std::uint32_t *seg_bucket_max, const std::uint32_t *seg_bucket_live,
...
__device__ inline std::uint32_t seg_range_sum_one(
    std::uint32_t ord, std::uint32_t seg_id, std::uint32_t lo,
    std::uint32_t hi, const std::uint32_t *pool_keys,
    const std::uint32_t *pool_values, const std::uint32_t *seg_bucket_max,
...
__device__ inline std::uint32_t seg_sheet_range_sum(
  ...
  if (pr >= dir_count)
    pr = dir_count - 1;
  if (pl == pr) {
    return seg_range_sum_one(static_cast<std::uint32_t>(pl), dir_seg_id[pl],
                             lo, hi, pool_keys, pool_values, seg_bucket_max,
                             seg_bucket_live, seg_bucket_value_prefix, epochs,
                             epoch_count);
  }
  std::uint32_t sum =
      seg_range_sum_one(static_cast<std::uint32_t>(pl), dir_seg_id[pl], lo,
                        hi, pool_keys, pool_values, seg_bucket_max,
                        seg_bucket_live, seg_bucket_value_prefix, epochs,
                        epoch_count) +
      seg_range_sum_one(static_cast<std::uint32_t>(pr), dir_seg_id[pr], lo,
                        hi, pool_keys, pool_values, seg_bucket_max,
                        seg_bucket_live, seg_bucket_value_prefix, epochs,
                        epoch_count);
  if (pr > pl + 1)
...
__device__ inline std::uint32_t seg_sheet_range_count(
  ...
  if (pr >= dir_count)
    pr = dir_count - 1;
  if (pl == pr) {
    return seg_range_count_one(static_cast<std::uint32_t>(pl), dir_seg_id[pl],
                               lo, hi, pool_keys, seg_bucket_max,
                               seg_bucket_live, seg_bucket_live_prefix, epochs,
                               epoch_count);
  }
  std::uint32_t total =
      seg_range_count_one(static_cast<std::uint32_t>(pl), dir_seg_id[pl], lo,
                          hi, pool_keys, seg_bucket_max, seg_bucket_live,
                          seg_bucket_live_prefix, epochs, epoch_count) +
      seg_range_count_one(static_cast<std::uint32_t>(pr), dir_seg_id[pr], lo,
                          hi, pool_keys, seg_bucket_max, seg_bucket_live,
                          seg_bucket_live_prefix, epochs, epoch_count);
  if (pr > pl + 1)
...
__global__ void epoch_quotient_metadata_kernel(
  ...
  for (std::uint32_t i = tid; i < record_count; i += stride) {
    ...
    while (end < record_count &&
           (keys[end] >> kEpochQuotientBits) == quotient)
      ++end;
    if (i == 0u) {
    ...
    if (count <= 32u) {
      for (std::uint32_t p = i; p < end; ++p) {
        const std::uint32_t subgroup =
            (keys[p] >> (kEpochQuotientBits - kEpochSubgroupBits)) &
            (kEpochSubgroups - 1);
        const std::uint32_t position = 1u << (p - i);
#pragma unroll
...
__global__ void epoch_quotient_value_sum_kernel(
    const EpochView *epochs, int epoch_count, std::uint32_t pending_mask) {
  const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
  const std::uint32_t total =
      static_cast<std::uint32_t>(epoch_count) * kEpochQuotients;
  if (index >= total)
    return;
  const int epoch_index = index >> kEpochQuotientBits;
  if ((pending_mask & (1u << epoch_index)) == 0u)
    return;
  const std::uint32_t quotient = index & (kEpochQuotients - 1);
  const EpochView ev = epochs[epoch_index];
  const std::uint32_t begin = ev.quotient_off[quotient];
  const std::uint32_t end = ev.quotient_off[quotient + 1];
  std::uint32_t sum = 0u;
  for (std::uint32_t p = begin; p < end; ++p)
...
__global__ void epoch_classify_heavy_quotients_kernel(
    const EpochView *epochs, int epoch_count, std::uint32_t pending_mask) {
  const std::uint32_t index = blockIdx.x * blockDim.x + threadIdx.x;
  const std::uint32_t total =
      static_cast<std::uint32_t>(epoch_count) * kEpochQuotients;
...
__global__ void epoch_sort_heavy_quotients_kernel(
  ...
  const int epoch_index = blockIdx.x / shards;
  if (epoch_index >= epoch_count ||
      (pending_mask & (1u << epoch_index)) == 0u)
    return;
  const int shard = blockIdx.x - epoch_index * shards;
  const EpochView ev = epochs[epoch_index];
...
__global__ void epoch_spine_delete_kernel(
    ...
    int epoch_count, std::uint32_t *epoch_dirty,
    std::uint32_t *epoch_removed, const std::uint32_t *spine_keys,
...
__global__ void epoch_gather_live_kernel(
    const std::uint32_t *keys, const std::uint32_t *values,
    const std::uint8_t *dead, const std::uint32_t *count_prefix,
    std::size_t n, std::uint32_t output_base, std::uint32_t *out_keys,
    std::uint32_t *out_values) {
  const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n || dead[i])
    return;
...
__global__ void point_lookup_walk_kernel(
    ...
    int num_runs, const std::uint32_t *spine_keys,
    const std::uint32_t *spine_values, const std::uint32_t *spine_radix,
    ...
    const std::uint32_t *pool_keys, const std::uint32_t *pool_values,
    const std::uint32_t *seg_bucket_max,
    const std::uint32_t *seg_bucket_live, const EpochView *epochs,
    int epoch_count) {
  const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
  ...
  for (int r = 0; r < num_runs; ++r) {
    ...
    if (p < rc && rk[p] == key) {
      ...
      out_value[o] = live ? val : kEmptyKey;
      return;
    }
  }
  ...
  if (!found && spine_count == 0 && dir_count > 0) {
    std::size_t ord =
        dir_route(key, dir_radix_first, dir_boundary, dir_count);
    if (ord >= dir_count)
      ord = dir_count - 1;
    found = seg_point_lookup(static_cast<std::uint32_t>(ord), dir_seg_id[ord],
                             key, pool_keys, pool_values, seg_bucket_max,
                             seg_bucket_live, nullptr, 0, &value);
  }
  out_found[o] = found ? 1u : 0u;
  out_value[o] = found ? value : kEmptyKey;
}

__global__ void seg_flip_lookup_kernel(
    const std::uint32_t *sorted_q, const std::uint32_t *ord_arr,
    const std::uint32_t *seg_id_arr,
  ...
  for (int b = threadIdx.x; b < kSegmentBuckets; b += blockDim.x) {
    ...
    for (std::size_t qi = qb + qbb; qi < qb + qee; ++qi) {
      ...
      const std::uint32_t value =
          found ? pool_values[base + ki] : kEmptyKey;
      out_found[qi] = found;
      out_value[qi] = value;
    }
  }
...
__global__ void overlay_walk_override_kernel(
  ...
  for (int r = 0; r < num_runs; ++r) {
    ...
    if (p < rc && rk[p] == q) {
      ...
      out_value[i] = live ? val : kEmptyKey;
      return;
    }
  }
}
...
__global__ void successor_index_kernel(
  ...
  if (spine_count == 0 && dir_count > 0 && q < best) {
...
__global__ void sheet_point_values_kernel(
    ...
    std::size_t spine_count, const std::uint32_t *dir_boundary,
    const std::uint32_t *dir_radix_first, const std::uint32_t *dir_seg_id,
    std::size_t dir_count,
    const std::uint32_t *pool_keys, const std::uint32_t *pool_values,
    const std::uint32_t *seg_bucket_max,
    const std::uint32_t *seg_bucket_live, const EpochView *epochs,
    int epoch_count, std::uint32_t *out_val,
    std::uint32_t *out_flag) {
  const std::size_t i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n)
    return;
  ...
  if (!f && spine_count == 0 && dir_count > 0) {
    std::size_t ord =
        dir_route(keys[i], dir_radix_first, dir_boundary, dir_count);
    if (ord >= dir_count)
      ord = dir_count - 1;
    f = seg_point_lookup(static_cast<std::uint32_t>(ord), dir_seg_id[ord],
                         keys[i], pool_keys, pool_values, seg_bucket_max,
                         seg_bucket_live, nullptr, 0, &v);
  }
  out_val[i] = f ? v : 0u;
  out_flag[i] = f ? 1u : 0u;
}
...
__global__ void range_overlay_kernel(
  ...
  std::uint32_t sum = spine_count > 0
                          ? spine_range_sum(
                                spine_keys, spine_values, spine_radix,
                                spine_value_prefix, spine_dead,
                                spine_dead_value_prefix, spine_count, l, h)
                          : seg_sheet_range_sum(
                                l, h, dir_radix_first, dir_boundary, dir_seg_id,
                                dir_value_prefix, dir_count, pool_keys,
                                pool_values, seg_bucket_max, seg_bucket_live,
  ...
  out_sums[i] = sum;
  if (out_counts) {
    std::uint32_t c = spine_count > 0
                          ? spine_range_count(
                                spine_keys, spine_radix, spine_dead,
                                spine_dead_count_prefix, spine_count, l, h)
                          : seg_sheet_range_count(
                                l, h, dir_radix_first, dir_boundary, dir_seg_id,
...
__global__ void count_overlay_kernel(
  ...
  std::uint32_t c = spine_count > 0
                        ...
                        : seg_sheet_range_count(
                              l, h, dir_radix_first, dir_boundary, dir_seg_id,
                              dir_prefix, dir_count, pool_keys, seg_bucket_max,
                              seg_bucket_live, seg_bucket_live_prefix, epochs,
                              epoch_count);
  for (int e = 0; e < epoch_count; ++e)
    c += epoch_range_count_one(epochs[e], l, h);
...
public:
  ...
  void count(const DeviceRangeBatch &batch, cudaStream_t stream) {
    ...
    if (epoch_heavy_pending() || epoch_count_prefix_pending() ||
        (spine_has_dead_ && !spine_dead_prefix_ready_)) {
      guard.unlock();
      exclusive.lock();
      ensure_epoch_heavy_sorted(stream);
      ensure_epoch_count_prefixes(stream);
      ensure_spine_dead_prefixes(stream);
    }
  ...
  void successor(const DeviceSuccessorBatch &batch, cudaStream_t stream) {
    ...
    std::unique_lock<std::shared_mutex> exclusive(snapshot_mutex_,
                                                   std::defer_lock);
    if (epoch_heavy_pending() || epoch_bitmap_pending()) {
      guard.unlock();
      exclusive.lock();
      ensure_epoch_heavy_sorted(stream);
    ...
    gpulsmopt_detail::successor_index_kernel<<<grid, block, 0, stream>>>(
        ...
        h_dir_seg_id_.size(),
        raw_or_null(pool_keys_), raw_or_null(seg_bucket_max_),
        raw_or_null(seg_bucket_live_), epoch_view_ptr(), epoch_count(), killed,
        killed_count, live_ins,
        live_ins_count);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
  ...
  void range(const DeviceRangeOutputBatch &batch, cudaStream_t stream) {
    ...
    gpulsmopt_detail::range_overlay_kernel<<<grid, block, 0, stream>>>(
        ...
        spine_count_, raw_or_null(d_dir_boundary_),
        ...
        raw_or_null(seg_bucket_value_prefix_), epoch_view_ptr(), epoch_count(),
  ...
  void bulk_build(const std::uint32_t *keys, const std::uint32_t *values,
    ...
    thrust::sort_by_key(policy, scratch_incoming_keys_.begin(),
                        ...
                        scratch_incoming_values_.begin());
    install_spine(raw_or_null(scratch_incoming_keys_),
                  raw_or_null(scratch_incoming_values_), n, stream);
  ...
  std::size_t gpu_resident_bytes() const {
    ...
    std::size_t total = device_bytes_all(
        ...
        d_dir_radix_first_, epoch_views_,
...
private:
  ...
  void install_spine(const std::uint32_t *keys,
                     const std::uint32_t *values, std::size_t count,
                     cudaStream_t stream) {
    spine_keys_.resize_discard(count);
    spine_values_.resize_discard(count);
    if (count > 0) {
      CUDA_CHECK(cudaMemcpyAsync(spine_keys_.data(), keys,
                                 count * sizeof(std::uint32_t),
                                 cudaMemcpyDeviceToDevice, stream));
      CUDA_CHECK(cudaMemcpyAsync(spine_values_.data(), values,
                                 count * sizeof(std::uint32_t),
                                 cudaMemcpyDeviceToDevice, stream));
    }
    spine_count_ = count;
  ...
  void ensure_spine_dead_prefixes(cudaStream_t stream) {
    if (!spine_has_dead_ || spine_dead_prefix_ready_)
      return;
  ...
  struct EpochStorage {
    ...
    std::size_t slot_id = std::numeric_limits<std::size_t>::max();
    bool has_dead = false;
  ...
  struct EpochMetadataGraph {
    cudaGraph_t graph = nullptr;
    cudaGraphExec_t executable = nullptr;
    gpulsmopt_detail::EpochView view{};
    std::size_t count = 0;
  };
  ...
  bool metadata_graph_matches(const EpochMetadataGraph &state,
                              EpochStorage &epoch,
                              std::size_t count) {
    auto view = make_epoch_view(epoch);
    view.has_dead = 0u;
    return state.executable != nullptr && state.count == count &&
           same_epoch_view(state.view, view);
  }

  void destroy_epoch_metadata_graph(EpochMetadataGraph &state) noexcept {
    if (state.executable)
  ...
  void destroy_epoch_metadata_graphs() noexcept {
    for (auto &state : epoch_metadata_graphs_)
      destroy_epoch_metadata_graph(state);
  }

  void capture_epoch_metadata_graph(EpochStorage &epoch, std::size_t count,
                                    cudaStream_t stream) {
    if (epoch.slot_id >= epoch_metadata_graphs_.size())
      return;
    EpochMetadataGraph &state = epoch_metadata_graphs_[epoch.slot_id];
    destroy_epoch_metadata_graph(state);
    CUDA_CHECK(cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal));
    launch_epoch_metadata_kernels(epoch, count, stream);
    cudaGraph_t graph = nullptr;
    ...
    CUDA_CHECK(cudaStreamSynchronize(stream));
    state.view = make_epoch_view(epoch);
    state.view.has_dead = 0u;
    state.count = count;
  }

  void prepare_epoch_metadata_graphs(std::size_t count) {
    if (count < static_cast<std::size_t>(GPULSMOPT_SCATTER_MIN_BATCH) ||
        count > std::numeric_limits<std::uint32_t>::max())
      return;
    cudaStream_t capture_stream = nullptr;
    CUDA_CHECK(cudaStreamCreateWithFlags(&capture_stream,
                                          cudaStreamNonBlocking));
    try {
      for (auto &epoch : epochs_)
        capture_epoch_metadata_graph(epoch, count, capture_stream);
  ...
  bool launch_epoch_metadata_graph(EpochStorage &epoch, cudaStream_t stream) {
    if (epoch.slot_id >= epoch_metadata_graphs_.size())
      return false;
    EpochMetadataGraph &state = epoch_metadata_graphs_[epoch.slot_id];
    if (!metadata_graph_matches(state, epoch, epoch.count))
      return false;
    CUDA_CHECK(cudaGraphLaunch(state.executable, stream));
    return true;
  }

  void commit_epoch_metadata(EpochStorage &epoch, cudaStream_t stream) {
    prepare_epoch_metadata_storage(epoch);
    if (!launch_epoch_metadata_graph(epoch, stream))
  ...
  void ensure_epoch_heavy_sorted(cudaStream_t stream) {
    ...
    for (std::size_t i = 0; i < epochs_.size(); ++i)
      if (!epochs_[i].heavy_sorted) {
        pending_mask |= 1u << i;
        CUDA_CHECK(cudaMemsetAsync(epochs_[i].heavy_count.data(), 0,
                                   sizeof(std::uint32_t), stream));
      }
    if (pending_mask == 0u)
      return;
    constexpr int block = 256;
    const std::size_t quotient_count =
        epochs_.size() * gpulsmopt_detail::kEpochQuotients;
    const int classify_grid =
        static_cast<int>((quotient_count + block - 1) / block);
    gpulsmopt_detail::epoch_classify_heavy_quotients_kernel<<<
        classify_grid, block, 0, stream>>>(epoch_view_ptr(), epoch_count(),
                                           pending_mask);
    CUDA_CHECK(cudaGetLastError());
    constexpr int shards = 64;
    const int grid = epoch_count() * shards;
    gpulsmopt_detail::epoch_sort_heavy_quotients_kernel<<<grid, 128, 0,
                                                          stream>>>(
        epoch_view_ptr(), epoch_count(), pending_mask);
    CUDA_CHECK(cudaGetLastError());
    for (auto &epoch : epochs_)
  ...
  void begin_epoch_deletes(const std::uint32_t *keys, std::size_t count,
    ...
    if (ecount > 0) {
      CUDA_CHECK(cudaMemsetAsync(raw_or_null(epoch_dirty_), 0,
                                 ecount * sizeof(std::uint32_t), stream));
      CUDA_CHECK(cudaMemsetAsync(raw_or_null(epoch_removed_), 0,
                                 ecount * sizeof(std::uint32_t), stream));
    }
    spine_removed_.resize_discard(1);
    CUDA_CHECK(cudaMemsetAsync(spine_removed_.data(), 0,
                               sizeof(std::uint32_t), stream));
    sort_delete_batch(keys, count, stream);
    const int block = 256;
    const int grid = static_cast<int>((count + block - 1) / block);
    gpulsmopt_detail::epoch_spine_delete_kernel<<<grid, block, 0, stream>>>(
        delete_sort_keys_.data(), count, epoch_view_ptr(), epoch_count(),
        raw_or_null(epoch_dirty_), raw_or_null(epoch_removed_),
    ...
    if (ecount > 0) {
      CUDA_CHECK(cudaMemcpyAsync(epoch_dirty_host_.data(),
                                 raw_or_null(epoch_dirty_),
                                 ecount * sizeof(std::uint32_t),
    ...
    CUDA_CHECK(cudaMemcpyAsync(&spine_removed_host_, spine_removed_.data(),
                               sizeof(std::uint32_t),
                               cudaMemcpyDeviceToHost, stream));
  }
  ...
  void finish_epoch_deletes(cudaStream_t stream) {
    ...
    if (spine_removed_host_ > spine_live_count_)
      throw std::runtime_error("spine delete count overflow");
    if (spine_removed_host_ > 0) {
      spine_live_count_ -= spine_removed_host_;
      spine_has_dead_ = true;
      spine_dead_prefix_ready_ = false;
    }
    if (!epochs_.empty() || output == 0)
  ...
  void acquire_epoch_slot() {
    if (epoch_pool_.empty()) {
      ...
      epochs_.back().slot_id = next_epoch_slot_id_++;
      return;
    }
  ...
  void reserve_epoch_storage(std::size_t count) {
    while (epochs_.size() + epoch_pool_.size() <
      ...
      epoch_pool_.back().slot_id = next_epoch_slot_id_++;
    }
    for (auto &epoch : epoch_pool_) {
      epoch.keys.resize_discard(count);
      epoch.values.resize_discard(count);
  ...
  void recompute_sheet_live_count() {
    std::size_t total = spine_count_ > 0 ? spine_live_count_ : 0;
    if (spine_count_ == 0)
  ...
  void prepare_sort_storage(std::size_t direct_count,
    ...
    if (direct_count > 0) {
      ...
      CUDA_CHECK(cub::DeviceRadixSort::SortKeys(
          nullptr, delete_bytes, raw_or_null(scratch_incoming_keys_),
  ...
  void prepare_scatter_storage() {
    ...
    epoch_delete_found_.resize_discard(batch_hint);
  ...
  void prepare_for_insert(cudaStream_t stream) {
    ...
    const std::size_t warm_count =
        std::min({batch_size_, direct_count, scratch_incoming_keys_.size(),
                  scratch_incoming_values_.size()});
    if (warm_count >= static_cast<std::size_t>(
                          GPULSMOPT_SCATTER_MIN_BATCH)) {
      sort_epoch_batch(raw_or_null(scratch_incoming_keys_),
                       raw_or_null(scratch_incoming_values_), warm_count,
                       direct_sort_keys_.data(), direct_sort_values_.data(),
                       stream);
    }
    reserve_epoch_storage(direct_count);
    prepare_epoch_metadata_graphs(batch_size_);
    prebind_epoch_views(stream);
  ...
  void consolidate_all_epochs(cudaStream_t stream) {
    ...
    for (auto &epoch : epochs_) {
      ...
      if (!epoch.has_dead) {
        CUDA_CHECK(cudaMemcpyAsync(
            epoch_merge_keys_.data() + output_base, epoch.keys.data(),
            epoch.count * sizeof(std::uint32_t), cudaMemcpyDeviceToDevice,
            stream));
        CUDA_CHECK(cudaMemcpyAsync(
            epoch_merge_values_.data() + output_base, epoch.values.data(),
            epoch.count * sizeof(std::uint32_t), cudaMemcpyDeviceToDevice,
            stream));
      } else {
        const int grid =
        ...
        gpulsmopt_detail::epoch_gather_live_kernel<<<grid, block, 0, stream>>>(
            ...
            static_cast<std::uint32_t>(output_base), epoch_merge_keys_.data(),
            epoch_merge_values_.data());
        CUDA_CHECK(cudaGetLastError());
      }
      output_base += epoch.live_count;
    }
    if (live_total > 0)
    ...
    if (spine_live_count_ == 0 && live_total > 0) {
      CUDA_CHECK(cudaMemcpyAsync(
          spine_merge_keys_.data(), direct_sort_keys_.data(),
          live_total * sizeof(std::uint32_t), cudaMemcpyDeviceToDevice, stream));
      CUDA_CHECK(cudaMemcpyAsync(
          spine_merge_values_.data(), direct_sort_values_.data(),
          live_total * sizeof(std::uint32_t), cudaMemcpyDeviceToDevice, stream));
    } else if (live_total == 0 && spine_live_count_ > 0) {
      CUDA_CHECK(cudaMemcpyAsync(
  ...
  void sheet_delta_insert(const std::uint32_t *sorted_keys,
                          ...
                          std::size_t count, cudaStream_t stream) {
    if (h_dir_seg_id_.empty() || count == 0)
      return;
    {
      GPULSMOPT_PROF_PHASE(prof_delta_ingest_ms_);
  ...
  bool try_scatter_sheet_delete(const std::uint32_t *keys_in,
    ...
    if (h_dir_seg_id_.empty())
      return false;
    if (spine_count_ > 0 || !epochs_.empty()) {
    ...
    if (c0_log_count_ > 0 || !runs_.empty())
      merge_down(stream);
    ensure_host_directory_metadata(stream);
    const bool had_epochs = !epochs_.empty();
    const std::size_t dir_count = h_dir_seg_id_.size();
    const std::size_t total_buckets =
        dir_count * static_cast<std::size_t>(gpulsmopt_detail::kSegmentBuckets);
    ...
    const std::size_t dirty_cap = std::min(count, total_buckets);
    {
      ...
      CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
          ...
          static_cast<int>(total_buckets), stream));
      if (had_epochs) {
        gpulsmopt_detail::ds_scatter_epoch_misses_kernel<<<
            key_grid, block, 0, stream>>>(
            keys_in, count, ds_dest_.data(), epoch_delete_found_.data(),
            ds_base_.data(), cursor, ds_staged_keys_.data());
      } else {
    ...
    }
    {
      ...
      const int delete_grid =
          static_cast<int>(std::max<std::size_t>(
              1, std::min<std::size_t>(needed_blocks, 1024)));
    ...
    printf("[prof]     scatter_delete: dirty_buckets=%u (batch=%zu)\n",
           num_dirty, count);
#endif
    if (sheet_fragmented_ && !had_epochs) {
      merge_underfull_segments(stream);
      sheet_fragmented_ = false;
    }
    return true;
  ...
  void build_overlay_read_index(OverlayReadIndex &ix, cudaStream_t stream) {
    ...
    if (tomb > 0 && h_dir_seg_id_.size() > 0) {
      ...
      gpulsmopt_detail::sheet_point_values_kernel<<<grid, block, 0, stream>>>(
          raw_or_null(ix.gk) + ins, tomb, spine_keys_.data(),
          spine_values_.data(), spine_radix_.data(),
          spine_has_dead_ ? spine_dead_words_.data() : nullptr, spine_count_,
          raw_or_null(d_dir_boundary_),
          raw_or_null(d_dir_radix_first_), raw_or_null(d_dir_seg_id_),
          h_dir_seg_id_.size(),
          raw_or_null(pool_keys_), raw_or_null(pool_values_),
          raw_or_null(seg_bucket_max_), raw_or_null(seg_bucket_live_),
  ...
  void lookup_flip(std::size_t n, const std::uint32_t *const *rk,
    ...
    gpulsmopt_detail::seg_flip_lookup_kernel<<<static_cast<unsigned>(m), 256, 0,
                                               stream>>>(
        raw_or_null(lookup_sorted_queries_), raw_or_null(seg_dirty_ord_),
        raw_or_null(seg_d_dirty_seg_id_),
  ...
  void lookup_layered(const DeviceLookupBatch &batch, cudaStream_t stream) {
    ...
    const std::size_t nbuckets =
        ...
        static_cast<std::size_t>(gpulsmopt_detail::kSegmentBuckets);
    const bool use_flip =
        spine_count_ == 0 && !h_dir_seg_id_.empty() && n >= nbuckets &&
    ...
    if (!use_flip && num_runs == 0) {
      ...
      gpulsmopt_detail::point_lookup_walk_kernel<<<grid, block, 0, stream>>>(
          ...
          ro, rc, 0, spine_keys_.data(), spine_values_.data(),
          spine_radix_.data(),
          spine_has_dead_ ? spine_dead_words_.data() : nullptr, spine_count_,
          raw_or_null(d_dir_boundary_),
          raw_or_null(d_dir_radix_first_), raw_or_null(d_dir_seg_id_),
          ...
          raw_or_null(seg_bucket_live_), epoch_view_ptr(), epoch_count());
      CUDA_CHECK(cudaGetLastError());
      return;
    }
  ...
  bool spine_dead_prefix_ready_ = true;
  std::uint32_t spine_removed_host_ = 0;
  ...
  std::size_t next_epoch_slot_id_ = 0;
  ...
  gpulsmopt_detail::RawDeviceBuffer<std::uint32_t> spine_removed_;
