...
class SlabAllocLightContext {
 public:
  ...
  static constexpr uint32_t MEM_BLOCK_BIT_OFFSET_ALLOC_ = 10; // this is fixed
  static constexpr uint32_t SUPER_BLOCK_BIT_OFFSET_ALLOC_ = 10 + LOG_NUM_MEM_BLOCKS_;
  ...
  // jh: this was a constexpr before
  ...
  __device__ __host__ void initParameters(uint32_t* d_super_block, uint32_t hash_coef, uint32_t num_super_blocks) {
  ...
  getMemBlockIndex(SlabAllocAddressT address) const {
    return ((address >> MEM_BLOCK_BIT_OFFSET_ALLOC_) & (NUM_MEM_BLOCKS_PER_SUPER_BLOCK_ - 1));
  }
  ...
  __device__ __forceinline__ uint32_t warpAllocate(const uint32_t& laneId) {
    ...
    // jh: changed this to
    // allocated result:  remaining           bits: super_block_index
    //                    LOG_NUM_MEM_BLOCKS_ bits: memory block index
    //                    5                   bits: memory unit index (hi-bits of 10bit)
    //                    5                   bits: memory unit index (lo-bits of 10bit)
    int empty_lane = -1;
    uint32_t free_lane;
...
// jh: i changed "NUM_SUPER_BLOCKS_ALLOCATOR_" (defining the amount of super blocks to allocate) to "MAX_SUPER_BLOCKS_"
// jh: the actual amount of super blocks can now be set during runtime
// jh: for backwards compatibility, we use the max as the runtime default
// jh: the number of bits addressing the block within a super block is now determined by "LOG_NUM_MEM_BLOCKS_" (was hardcoded before)
/*
 * This class owns the memory for the allocator on the device
 */
template <uint32_t LOG_NUM_MEM_BLOCKS_,
          uint32_t MAX_SUPER_BLOCKS_,
          uint32_t MEM_UNIT_WARP_MULTIPLES_ = 1>
class SlabAllocLight {
  static_assert((size_t(1) << (10 + LOG_NUM_MEM_BLOCKS_)) * MAX_SUPER_BLOCKS_ <= (size_t(1) << 32u));
 ...
 public:
  ...
  // constructor:
  // =========
  SlabAllocLight(uint32_t num_super_blocks = MAX_SUPER_BLOCKS_) : d_super_blocks_(nullptr), hash_coef_(0) {
    if (num_super_blocks > MAX_SUPER_BLOCKS_) {
        throw std::runtime_error("number of super blocks exceeds limit");
    }
    ...
    CHECK_ERROR(cudaMalloc((void**)&d_super_blocks_,
                           slab_alloc_context_.SUPER_BLOCK_SIZE_ * num_super_blocks * sizeof(uint32_t)));
    ...
    slab_alloc_context_.initParameters(d_super_blocks_, hash_coef_, num_super_blocks);
  }
  ...
  using AllocatorContextT = SlabAllocLightContext<LOG_NUM_MEM_BLOCKS_, MEM_UNIT_WARP_MULTIPLES_>;
