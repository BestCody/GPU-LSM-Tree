...
class GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, AllocatorContextT> {
 public:
...
#pragma hd_warning_disable
  __host__ __device__ GpuSlabHashContext(
      GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, AllocatorContextT>& rhs) {
...
#pragma hd_warning_disable
  ...
  // jh: added this
  __device__ __forceinline__ void searchKeySum(bool& to_be_searched,
                                               const uint32_t& laneId,
                                               const KeyT& myKey,
                                               ValueT& myValueSum,
                                               const uint32_t bucket_id);
...
class GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT> {
 private:
  // fixed known parameters:
 ...
 public:
  ...
  GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, typename DynamicAllocatorT::AllocatorContextT> gpu_context_;
 private:
 ...
 public:
  GpuSlabHash(const uint32_t num_buckets,
    ...
    slab_unit_size_ =
        GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::getSlabUnitSize();
