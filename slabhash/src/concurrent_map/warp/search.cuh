...
//================================================
template <typename KeyT, typename ValueT, typename AllocatorContextT>
__device__ __forceinline__ void
GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, AllocatorContextT>::searchKey(
    bool& to_be_searched,
...
template <typename KeyT, typename ValueT, typename AllocatorContextT>
__device__ __forceinline__ void
GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, AllocatorContextT>::searchKeyBulk(
    const uint32_t& laneId,
    const KeyT& myKey,
    ValueT& myValue,
    const uint32_t bucket_id) {
...
// jh: added this, modified from count.cuh
template <typename KeyT, typename ValueT, typename AllocatorContextT>
__device__ __forceinline__ void
GpuSlabHashContext<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, AllocatorContextT>::searchKeySum(
    bool& to_be_searched,
    const uint32_t& laneId,
    const KeyT& myKey,
    ValueT& myValueSum,
  ...
  // jh: required for fast reduction
  ...
  while ((work_queue = __ballot_sync(0xFFFFFFFF, to_be_searched))) {
    ...
    bool is_key_lane = (uint32_t(1) << laneId) & SlabHashT::REGULAR_NODE_KEY_MASK;
    const auto value = __shfl_down_sync(0xFFFFFFFF, src_unit_data, 1);
    const auto value_to_sum = is_key_lane && src_unit_data == wanted_key ? value : 0;
    const auto sum = coop::reduce(tile, value_to_sum, coop::plus<ValueT>());
