...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::buildBulk(
    KeyT* d_key,
    ValueT* d_value,
    uint32_t num_keys) {
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::buildBulkWithUniqueKeys(
    KeyT* d_key,
    ValueT* d_value,
    uint32_t num_keys) {
  const uint32_t num_blocks = (num_keys + BLOCKSIZE_ - 1) / BLOCKSIZE_;
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::searchIndividual(
    KeyT* d_query,
    ValueT* d_result,
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::searchBulk(
    KeyT* d_query,
    ValueT* d_result,
    uint32_t num_queries) {
  CHECK_CUDA_ERROR(cudaSetDevice(device_idx_));
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::countIndividual(
    KeyT* d_query,
    uint32_t* d_count,
    uint32_t num_queries) {
  CHECK_CUDA_ERROR(cudaSetDevice(device_idx_));
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::deleteIndividual(
    KeyT* d_key,
    uint32_t num_keys) {
  CHECK_CUDA_ERROR(cudaSetDevice(device_idx_));
  const uint32_t num_blocks = (num_keys + BLOCKSIZE_ - 1) / BLOCKSIZE_;
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
void GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::batchedOperation(
    KeyT* d_key,
    ValueT* d_result,
    uint32_t num_ops) {
  CHECK_CUDA_ERROR(cudaSetDevice(device_idx_));
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
std::string GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::to_string() {
  std::string result;
  result += " ==== GpuSlabHash: \n";
...
template <typename KeyT, typename ValueT, typename DynamicAllocatorT>
double GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap, DynamicAllocatorT>::computeLoadFactor(
    int flag = 0) {
