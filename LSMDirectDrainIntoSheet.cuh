...
#endif
#ifndef GPULSMOPT_HOT_BUFFER_CAPACITY
#define GPULSMOPT_HOT_BUFFER_CAPACITY 4194304
#endif
...
class GPULSMOpt {
 public:
  ...
  explicit GPULSMOpt(const DictionaryConfig& config)
      : max_elements_(config.max_elements),
        ...
        hot_buffer_capacity_(std::max<std::size_t>(
            config.batch_size,
            static_cast<std::size_t>(GPULSMOPT_HOT_BUFFER_CAPACITY))) {
  ...
  void insert(const DeviceKeyValueBatch& batch, cudaStream_t stream) {
    ...
    std::unique_lock<std::shared_mutex> guard(snapshot_mutex_);
    append_constant_hot_op(batch.keys, batch.values, batch.count,
                           gpulsmopt_detail::kInsert, stream);
    maybe_flush_hot_buffer(stream);
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
  ...
  void update(const DeviceUpdateBatch& batch, cudaStream_t stream) {
    if (batch.count == 0) return;
    std::unique_lock<std::shared_mutex> guard(snapshot_mutex_);
    append_hot_update(batch, stream);
    maybe_flush_hot_buffer(stream);
    CUDA_CHECK(cudaStreamSynchronize(stream));
  }
  ...
  void erase(const DeviceKeyBatch& batch, cudaStream_t stream) {
    if (batch.count == 0) return;
    std::unique_lock<std::shared_mutex> guard(snapshot_mutex_);
    append_constant_hot_op(batch.keys, nullptr, batch.count,
  ...
  std::size_t gpu_resident_bytes() const {
    ...
    total += device_bytes_all(hot_keys_, hot_values_, hot_ops_, hot_seqs_);
 ...
 private:
  ...
  void widen_ops(const std::uint8_t* input,
                 ...
                 cudaStream_t stream) {
    widen_ops_into(input, output, 0, output.size(), stream);
  }

  void widen_ops_into(const std::uint8_t* input,
                      thrust::device_vector<std::uint32_t>& output,
    ...
    gpulsmopt_detail::widen_ops_kernel<<<grid, block, 0, stream>>>(
        input, raw_or_null(output) + offset, count);
    CUDA_CHECK(cudaGetLastError());
  }
  ...
  std::uint32_t allocate_sequence() {
    return allocate_sequence_range(1);
  }

  std::uint32_t allocate_sequence_range(std::size_t count) {
    if (count == 0) return next_seq_;
    if (count > static_cast<std::size_t>(0x7fffffffu - next_seq_)) {
      throw std::overflow_error(
          "GPULSMOpt sequence space exhausted; rebuild with wider packing");
    }
    const std::uint32_t first = next_seq_;
    next_seq_ += static_cast<std::uint32_t>(count);
    return first;
  }

  void clear_hot_buffer() {
    hot_keys_.clear();
    hot_values_.clear();
    hot_ops_.clear();
    hot_seqs_.clear();
  ...
  void append_constant_hot_op(const std::uint32_t* keys,
                              const std::uint32_t* values, std::size_t count,
                              std::uint32_t op, cudaStream_t stream) {
    const std::size_t offset = hot_count_;
    const std::size_t next = offset + count;
    const std::uint32_t seq_start = allocate_sequence_range(count);
    auto policy = thrust::cuda::par.on(stream);

    hot_keys_.resize(next);
    hot_values_.resize(next);
    hot_ops_.resize(next);
    hot_seqs_.resize(next);

    CUDA_CHECK(cudaMemcpyAsync(raw_or_null(hot_keys_) + offset, keys,
                               count * sizeof(std::uint32_t),
                               cudaMemcpyDeviceToDevice, stream));
    if (values) {
      CUDA_CHECK(cudaMemcpyAsync(raw_or_null(hot_values_) + offset, values,
                                 count * sizeof(std::uint32_t),
                                 cudaMemcpyDeviceToDevice, stream));
    } else {
      thrust::fill(policy, hot_values_.begin() + offset, hot_values_.end(), 0u);
    }
    thrust::fill(policy, hot_ops_.begin() + offset, hot_ops_.end(), op);
    thrust::sequence(policy, hot_seqs_.begin() + offset, hot_seqs_.end(),
                     seq_start);
    hot_count_ = next;
  }

  void append_hot_update(const DeviceUpdateBatch& batch, cudaStream_t stream) {
    const std::size_t offset = hot_count_;
    const std::size_t next = offset + batch.count;
    const std::uint32_t seq_start = allocate_sequence_range(batch.count);
    auto policy = thrust::cuda::par.on(stream);
    ...
    CUDA_CHECK(cudaMemcpyAsync(raw_or_null(hot_keys_) + offset, batch.keys,
                               batch.count * sizeof(std::uint32_t),
                               cudaMemcpyDeviceToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(raw_or_null(hot_values_) + offset, batch.values,
                               batch.count * sizeof(std::uint32_t),
                               cudaMemcpyDeviceToDevice, stream));
    widen_ops_into(batch.ops, hot_ops_, offset, batch.count, stream);
    thrust::sequence(policy, hot_seqs_.begin() + offset, hot_seqs_.end(),
                     seq_start);
    hot_count_ = next;
  }

  std::size_t build_resolved_hot_incoming_device(cudaStream_t stream) {
    const std::size_t total = hot_count_;
    if (total == 0) return 0;
    ...
    thrust::copy(policy, hot_keys_.begin(), hot_keys_.end(),
                 scratch_incoming_keys_.begin());
    thrust::copy(policy, hot_values_.begin(), hot_values_.end(),
                 scratch_incoming_values_.begin());
    thrust::copy(policy, hot_ops_.begin(), hot_ops_.end(),
                 scratch_incoming_ops_.begin());
    thrust::copy(policy, hot_seqs_.begin(), hot_seqs_.end(),
                 scratch_incoming_seqs_.begin());
    fill_drain_sort_keys(scratch_incoming_keys_, scratch_incoming_seqs_,
                         scratch_sort_keys_, 0, total, stream);
  ...
  void flush_hot_buffer_to_sheet(cudaStream_t stream) {
    if (hot_count_ == 0) return;
    const std::size_t incoming_count = build_resolved_hot_incoming_device(stream);
    clear_hot_buffer();
    apply_resolved_incoming_to_segments(incoming_count, stream);
  }

  void maybe_flush_hot_buffer(cudaStream_t stream) {
    if (hot_buffer_capacity_ > 0 && hot_count_ >= hot_buffer_capacity_) {
      flush_hot_buffer_to_sheet(stream);
    }
  }

  void flush_pending_to_sheet(cudaStream_t stream) {
    flush_hot_buffer_to_sheet(stream);
  ...
  void apply_resolved_incoming_to_segments(std::size_t incoming_count,
                                           cudaStream_t stream) {
  ...
  bool run_data_exceeds_budget() const {
    ...
    const std::size_t threshold =
        std::max<std::size_t>(run_drain_floor_, total / 20);
  ...
  std::size_t hot_buffer_capacity_ = GPULSMOPT_HOT_BUFFER_CAPACITY;
  ...
  thrust::device_vector<std::uint32_t> hot_keys_;
  thrust::device_vector<std::uint32_t> hot_values_;
  thrust::device_vector<std::uint32_t> hot_ops_;
  thrust::device_vector<std::uint32_t> hot_seqs_;
